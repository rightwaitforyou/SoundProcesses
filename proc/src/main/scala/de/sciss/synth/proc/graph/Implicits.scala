...
//      stream.VDiskIn(audio, key = buf.key, speed = speed, loop = loop, sendID = sendID)
//  }
//
//  /** Enables the use of a `stream` argument in place of the buf argument in regular `BufChannels` use. */
//  implicit class wrapBufChannels(peer: ugen.BufChannels.type) {
...
//  implicit class wrapBufRateScale(peer: ugen.BufRateScale.type) {
...
//  implicit class wrapBufSampleRateStream(peer: ugen.BufSampleRate.type) {
