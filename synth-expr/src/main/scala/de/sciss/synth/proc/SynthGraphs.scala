...
import de.sciss.{synth, lucre, model}
...
object SynthGraphs extends BiTypeImpl[SynthGraph] {
  ...
  def readValue(in: DataInput): SynthGraph = ??? // RRR impl.SynthGraphSerializer.read(in)

  def writeValue(value: SynthGraph, out: DataOutput): Unit =
    ??? // RRR lucre.synth.impl.SynthGraphSerializer.write(value, out)
  ...
  private lazy val oldTapeSynthGraph: SynthGraph = ??? // RRR
  //    SynthGraph {
  //      import synth._
  //      import ugen._
  ...
  private lazy val tapeSynthGraph: SynthGraph = ??? // RRR
  //    SynthGraph {
  //      import synth._
  //      val sig   = graph.scan.In(ProcKeys.graphAudio)
