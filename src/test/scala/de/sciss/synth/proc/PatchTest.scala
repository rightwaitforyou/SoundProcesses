...
object PatchTest extends App {

   {
      ...
      sys.root( _ => () )
      val auralSys = AuralSystem.start()
      auralSys.whenStarted( _ => run[ S, I ]( auralSys ))
   }

   def run[ S <: evt.Sys[ S ], I <: stm.Sys[ I ]]( auralSys: AuralSystem )
                                                 ( implicit cursor: stm.Cursor[ S ], bridge: S#Tx => I#Tx ) {
      ...
      cursor.step { implicit tx =>
         ...
         AuralPresentation.run( trans, auralSys )
