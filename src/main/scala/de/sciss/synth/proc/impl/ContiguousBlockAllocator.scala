...
import concurrent.stm.{TSet, InTxn, Ref, TMap, TArray}
import collection.immutable.{SortedMap => ISortedMap}
import annotation.tailrec

object ContiguousBlockAllocator {
   /**
...
//         if( all.size > 1 ) {
            all.sliding( 2, 1 ).foreach { seq => val a = seq.head; val b = seq.last; require( a.touches( b ), "" + a + " does not touch " + b )}
//         }
