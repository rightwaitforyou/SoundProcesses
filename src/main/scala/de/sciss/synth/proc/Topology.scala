...
final case class Topology[ V, E <: Topology.Edge[ V ]]( vertices: IIdxSeq[ V ], edges: ISet[ E ])
      ...
      if( upBound < unpositioned ) { // first edge for source
         if( loBound < unpositioned ) { // first edge for target
            ...
            val newVertices= vertices.patch( min, emptySeq, 1 ).patch( max - 1, emptySeq, 1 )
               .patch( newUnpos, IIdxSeq( source, target ), 0 )
            Some( (copy( newVertices, newEdgeSet )( newUnpos, newEdgeMap ), source, IIdxSeq( target )))
         } else {
//            Some( (this, emptySeq) )
            val newUnpos   = unpositioned - 1
            val sourceSeq  = IIdxSeq( source )
...
// XXX TEST
//      copy( vertices.patch( unpositioned, IIdxSeq( v ), 0 ), unpositioned + 1 )
