object Model {
   ...
   def onCommit[ C <: CtxLike[ _ ], T ]( committed: Traversable[ T ] => Unit ) : Listener[ C, T ] =
      filterOnCommit( (_: T, _: C) => true )( committed )

   def filterOnCommit[ C <: CtxLike[ _ ], T ]( filter: Function2[ T, C, Boolean ])( committed: Traversable[ T ] => Unit ) =
      new Listener[ C, T ] {
