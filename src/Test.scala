object Test {
   ...
   def test( p: Proc[ ECtx, ESystem.Var ])( implicit c: ECtx ) {
      p.switch.get
   }

   // possible to access a particular system
   // with a more specific context
   def test2( p: Proc[ ECtx, ESystem.Var ])( implicit c: KCtx ) {
      p.switch.get( c.eph )
   }
