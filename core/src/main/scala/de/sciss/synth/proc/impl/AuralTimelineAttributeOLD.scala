...
object AuralTimelineAttributeOLD extends Factory {
  ...
  def apply[S <: Sys[S]](key: String, value: Timeline[S], observer: Observer[S])
    ...
    new AuralTimelineAttributeOLD(key, tx.newHandle(value), observer).init(value)
}
final class AuralTimelineAttributeOLD[S <: Sys[S]](val key: String, val obj: stm.Source[S#Tx, Timeline[S]],
                                                observer: Observer[S])
