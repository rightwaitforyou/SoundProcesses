...
trait AuralTimelineBase_OLD[S <: Sys[S], I <: stm.Sys[I], Target, Elem <: AuralView[S, Target]]
...
//  private[this] implicit def itx(implicit tx: S#Tx): I#Tx = iSys(tx)
  ...
  import AuralTimelineBase_OLD.{LOOK_AHEAD, STEP_GRID, Scheduled, spanToPoint}
...
//  }
  ...
  private[this] def stopView(view: Elem)(implicit tx: S#Tx): Unit = {
    view.stop()
    view.dispose()
